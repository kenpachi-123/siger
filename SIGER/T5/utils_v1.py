...
def parse_train_args(parser):
    ...
    parser.add_argument("--use_cls_contra", action="store_true", default=False)
    parser.add_argument("--cls_contra_weight", type=float, default=0.1)
    parser.add_argument("--cls_temperature", type=float, default=0.1)
    parser.add_argument("--item_id_base", type=str, default="auto",
                        choices=["auto", "0", "1"],
                        help="item id base used by target_item_ids and sideinfo buffers")
    ...
    parser.add_argument("--use_codebook_init", action="store_true", default=False)
    
    parser.add_argument("--emb_file", type=str, default="")
    parser.add_argument("--cls_contra_type", type=str, default="infonce",
                        choices=["infonce", "cosine", "random_neg"])
    parser.add_argument("--cls_num_negatives", type=int, default=256,
                        help="number of randomly sampled negatives per positive "
                             "(only used when cls_contra_type='random_neg')")
    parser.add_argument("--cls_warmup_steps", type=int, default=0,
                        help="number of steps to linearly warm up cls_contra_weight from 0 to its target value")
    parser.add_argument("--cls_weight_scheduler", type=str, default="constant",
                        choices=["constant", "linear_warmup", "cosine_decay", "warmup_cosine", "constant_cosine"],
                        help="schedule for cls_contra_weight: "
                             "constant (no change), "
                             "linear_warmup (0->max over warmup_steps then constant), "
                             "cosine_decay (max->0 over total steps), "
                             "warmup_cosine (linear warmup then cosine decay to 0)")
