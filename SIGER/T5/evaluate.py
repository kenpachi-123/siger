...
def get_topk_results(predictions, scores, targets, k, all_items=None, dedup_predictions=False):
    results = []
    B = len(targets)
    ...
    for b in range(B):
        ...
        if dedup_predictions:
            # Optional evaluation fix:
            # For one sample, only the first exact match is allowed to contribute to
            # ranking metrics. Later repeated hits of the same target are forced to 0.
            # This preserves the original beam positions while preventing multiple
            # credits from being assigned to one ground-truth item.
            found_match = False
            for sorted_pred in sorted_pairs:
                if (not found_match) and sorted_pred[0] == target_item:
