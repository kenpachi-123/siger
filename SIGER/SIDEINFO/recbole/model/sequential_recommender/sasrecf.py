...
PRETRAINED_ITEM_EMB_DIM = 256
PRETRAINED_ITEM_EMB_PATH = "/root/blockdata/diffandletter/LETTER/data/Beauty/Beauty.ordered.npy"
...
class SASRecF(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if self.hidden_size != PRETRAINED_ITEM_EMB_DIM:
            raise ValueError(
                f"hidden_size must be {PRETRAINED_ITEM_EMB_DIM} to use the fixed pretrained item embedding, "
                f"but got {self.hidden_size}."
            )
        pretrained_emb = self._load_pretrained_item_embedding(dataset)
        ...
        if self.attribute_predictor == 'MLP':
            self.ap = nn.ModuleList([
                nn.Sequential(
                    ...
                    nn.ReLU(),
                    nn.Linear(in_features=self.hidden_size, out_features=self.n_attributes[feat])
    ...
    def _load_pretrained_item_embedding(self, dataset):
        if not os.path.exists(PRETRAINED_ITEM_EMB_PATH):
            raise FileNotFoundError(f"Pretrained embedding file not found: {PRETRAINED_ITEM_EMB_PATH}")

        raw_emb = np.load(PRETRAINED_ITEM_EMB_PATH)
        if raw_emb.ndim != 2:
            raise ValueError(f"Expected a 2D embedding matrix, but got shape {raw_emb.shape}.")
        if raw_emb.shape[1] < PRETRAINED_ITEM_EMB_DIM:
            raise ValueError(
                f"Pretrained embedding dim {raw_emb.shape[1]} is smaller than {PRETRAINED_ITEM_EMB_DIM}."
            )
        raw_emb = raw_emb[:, :PRETRAINED_ITEM_EMB_DIM]
        print(f"[INFO] Loaded pretrained embedding from file: shape {raw_emb.shape}")

        aligned = np.zeros((self.n_items, PRETRAINED_ITEM_EMB_DIM), dtype=raw_emb.dtype)
        rows_to_copy = min(raw_emb.shape[0], self.n_items - 1)
        aligned[1:1 + rows_to_copy] = raw_emb[:rows_to_copy]
        if raw_emb.shape[0] < self.n_items - 1:
            print(
                f"[WARN] Embedding rows ({raw_emb.shape[0]}) < required ({self.n_items - 1}); "
                "padding remaining rows with zeros."
            )
        elif raw_emb.shape[0] > self.n_items - 1:
            print(
                f"[WARN] Embedding rows ({raw_emb.shape[0]}) > required ({self.n_items - 1}); truncating."
            )
    ...
    def _init_weights(self, module):
        ...
        if module is self.item_embedding:
            return
