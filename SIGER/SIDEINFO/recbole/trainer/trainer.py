...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        # the following two members have been inherited.
        # self.config = config
        # self.model = model
        self.logger = getLogger()
        self.tensorboard = get_tensorboard(self.logger, config.dataset_name)
        ...
        self.alternative_train_batch = config['alternative_train_batch']
    ...
    def _log_extra_epoch_metrics(self, epoch_idx, verbose=True):
        if not hasattr(self.model, 'get_align_temperature'):
            align_temp = None
        else:
            align_temp = self.model.get_align_temperature()
            if isinstance(align_temp, torch.Tensor):
                align_temp = align_temp.detach().item()
            if verbose:
                self.logger.info(set_color('align temperature', 'blue') + f': {align_temp:.6f}')
            self.tensorboard.add_scalar('Train/align_temperature', align_temp, epoch_idx)

        if hasattr(self.model, 'get_item_emb_reg_temperature'):
            item_emb_reg_temp = self.model.get_item_emb_reg_temperature()
            if item_emb_reg_temp is not None:
                if isinstance(item_emb_reg_temp, torch.Tensor):
                    item_emb_reg_temp = item_emb_reg_temp.detach().item()
                if verbose:
                    self.logger.info(
                        set_color('item emb reg temperature', 'blue') + f': {item_emb_reg_temp:.6f}'
                    )
                self.tensorboard.add_scalar('Train/item_emb_reg_temperature', item_emb_reg_temp, epoch_idx)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        self.best_fusion_w = None  # shr para
        self.best_layer = None    # each layer has own fusion_w

        self.best_softmax_w = None
        self.best_soft_repr_w = None
        self.best_soft_wc = None
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            self._log_extra_epoch_metrics(epoch_idx, verbose=verbose)

            if self.config['model'] == 'MSSR':
                self.cur_softmax_w = self.model.trm_encoder.soft_fusion_w
                self.cur_soft_wc = self.model.trm_encoder.soft_fusion_w_c
        ...
        for epoch_idx in range(self.start_epoch, self.pretrain_epochs):
            ...
            if (epoch_idx + 1) % self.save_step == 0:
                saved_model_file = os.path.join(
                    self.checkpoint_dir, '{}-{}-{}.pth'.format(self.config['model'], self.config['dataset'], str(epoch_idx + 1))
                )
...
class S3RecTrainer(PretrainTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        print('****************')
        print()
        print('current stage: ', self.model.train_stage)
        print()
        print('================')
