...
class Trainer(object):
    ...
    def _build_label_cache(self, labels):
        label_tensor = torch.as_tensor(labels, device=self.device, dtype=torch.long)
        positive_pool, positive_pool_sizes = self.model.rq.vq_layers[0].build_positive_pool(label_tensor)
        return {
            "labels": label_tensor,
            "positive_pool": positive_pool,
            "positive_pool_sizes": positive_pool_sizes,
        }

    def _refresh_labels(self):
        embs  = [layer.embedding.weight.detach().cpu().numpy() for layer in self.model.rq.vq_layers]

        for idx, emb in enumerate(embs):
            labels = self.constrained_km(emb)
            self.labels[str(idx)] = self._build_label_cache(labels)
