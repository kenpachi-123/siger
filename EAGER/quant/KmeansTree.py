...
class ConstructKmeansTree:
            ...
            while dq:
                ...
                if len(index) == self.k:
                    for i in range(self.k):
                        id_code_list.append((index[i].item(), self.k * pcode + 1+i))
    ...
    def _kmeans(self,index):
        ...
        with torch.no_grad():
            ...
            selected_feature=torch.randperm(self.data.shape[1], device=self.data.device)[:math.ceil(self.feature_ratio*self.data.shape[1])]
            work_index = index.to(self.data.device)
            choices, _ = kmeans_equal(self.data[:,selected_feature][work_index], cluster_size=cluster_size,num_clusters=self.k,max_iters=self.max_iters)
        choices = choices.to(index.device)
